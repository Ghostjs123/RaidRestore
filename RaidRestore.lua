...
RR_CLASSES = {"warrior", "rogue", "warlock", "mage", "hunter", "shaman", "druid", "priest", "paladin"};
...
function RestoreRaid(strName)
	...
	GetRaidInfo(pSubRaid); -- NOTE: function redefined elsewhere
...
function RaidRestore_CommandHandler(msg)
	...
	for strCommand in string.gmatch(msg, "[^ ]+") do
...
function RaidRestore_OnLoad(self)
	SLASH_RR1 = "/rr";
	SlashCmdList["RR"] =
...
function QuickAccess_MainFrame_OnLoad(self)
	QuickAccess_Raid1.text = QuickAccess_Raid1:CreateFontString();
	QuickAccess_Raid1.text:SetTextColor(1.0, 1.0, 1.0, 1.0);
	QuickAccess_Raid1:SetFontString(QuickAccess_Raid1.text);

	QuickAccess_Raid2.text = QuickAccess_Raid2:CreateFontString();
	QuickAccess_Raid2.text:SetTextColor(1.0, 1.0, 1.0, 1.0);
	QuickAccess_Raid2:SetFontString(QuickAccess_Raid2.text);
end
...
function SetNullText(Control, strText, color)
	...
	if (color == true) then
		...
		if (color ~= nil) then
			Control.text = Control:CreateFontString();
			Control.text:SetTextColor(color.r, color.g, color.b);
			Control:SetFontString(Control.text);
		else
			Control.text = Control:CreateFontString();
			Control.text:SetTextColor(0.7, 0.7, 0.7);
			Control:SetFontString(Control.text);
		end
	end
...
function SetSelectedRaid(strName)
	if (strName ~= nil) then
		UIDropDownMenu_SetText(RaidRestore_TemplateList, string.lower(strName));
	end
end
...
function RaidRestore_TemplateList_OnLoad(self)
	UIDropDownMenu_Initialize(self, LoadTemplateList);
end

function RaidRestore_TemplateList_OnClick(self)
	SetSelectedRaid(self.value);
...
function SetSelectedFilter(strName)
	if (strName ~= nil) then
		UIDropDownMenu_SetText(RaidRestore_FilterList, string.lower(strName));
	end
end
...
function RaidRestore_FilterList_OnLoad(self)
	UIDropDownMenu_Initialize(self, LoadFilterList);
end

function RaidRestore_FilterList_OnClick(self)
	SetSelectedFilter(self.value);
...
function QuickAccess_FilterMenu_OnLoad(self)
	UIDropDownMenu_Initialize(self, LoadFilterMenuList, "MENU");
end

function QuickAccess_FilterMenu_OnClick(self)
	RaidRestore.SetOption("Filter", self.value);
	SetSelectedFilter(RaidRestore.GetOption("Filter"));	
end
...
function QuickAccess_RaidMenu_OnLoad(self)
	UIDropDownMenu_Initialize(self, LoadRaidList, "MENU");
end

function QuickAccess_RaidMenu_OnClick(self)
	if (RaidMenuParentButton == nil) then
		RestoreRaid(self.value);
	else
		SetNullText(RaidMenuParentButton, self.value, false);
	end
end
...
function RaidRestore_TypeMenu_OnLoad(self)
	UIDropDownMenu_Initialize(self, LoadTypeList, "MENU");
end

function RaidRestore_TypeMenu_OnClick(self)
	SetNullText(TypeMenuParentButton, self.value, true);
end
